...
def process_tables(root) -> None:
    for table in root.xpath('.//w:tbl', namespaces=NS):
        for row in table.xpath('./w:tr', namespaces=NS):
            ...
            elif 'Arranjo' in row_text:
                if len(cells) > 1 and not element_text(cells[1]).strip():
                    cells[1].append(etree.Element(f'{{{W_NS}}}p'))
                    paragraph = cells[1].xpath('./w:p', namespaces=NS)[-1]
                    run = etree.SubElement(paragraph, f'{{{W_NS}}}r')
                    text_node = etree.SubElement(run, f'{{{W_NS}}}t')
                    text_node.text = '{{TIPO_ARRANJO}}'
