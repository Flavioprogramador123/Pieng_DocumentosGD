...
| --- | --- |
| `gerar_documentos.py` | Lê o TXT, aplica o De/Para, normaliza os dados e preenche os templates. |
| `depara_chaves.json` | Arquivo editável com variações de rótulos e suas chaves canônicas. |
| `resolver_ambiguidades_ia.py` | Sugere mapeamentos para rótulos desconhecidos quando executado explicitamente. |
| `criar_template_procuracao.py` | Recria a procuração parametrizada a partir do DOCX original. |
| `criar_template_memorial.py` | Recria o Memorial Descritivo parametrizado a partir do DOCX preenchido. |
| `analise_memorial.md` | Mapeia os campos do Memorial e a responsabilidade de cada fonte de dados. |
...
O projeto foi escrito para Python 3.10 ou superior. Para usar o resolvedor opcional por IA, instale também `openai` e mantenha as variáveis de ambiente do serviço configuradas; sem executar esse resolvedor, o preenchimento principal funciona apenas com o tratamento local e não envia o TXT para a internet.
...
O sistema não depende de uma única forma de escrever o TXT. Primeiro ele tenta o rótulo exato; depois remove acentos, pontuação e diferenças de caixa; em seguida consulta `depara_chaves.json`. Assim, expressões como `UC`, `Unidade Consumidora`, `Número da UC` e `Conta Contrato` podem apontar para `CONTA_CONTRATO`.

Para endereços, o sistema aceita tanto `Endereço: Rua Pernambuco, Q. F, L. 5/6, Anexo Itamaraty` quanto campos separados como `Logradouro`, `Número`, `Quadra`, `Lote`, `Bairro` e `Cidade/UF`. Quando necessário, ele compõe `ENDERECO` e `ENDERECO_COMPLETO` sem apagar os componentes originais. Também converte quantidades como `32 unidades` em valores numéricos para as células do Excel.

O tratamento determinístico é sempre executado primeiro. Para rótulos que não forem reconhecidos, existe o resolvedor opcional `resolver_ambiguidades_ia.py`. Ele usa o modelo **gpt-5-mini** somente quando você executar o comando abaixo e grava sugestões em um JSON separado:

```bash
python resolver_ambiguidades_ia.py --input cliente_001.txt --output sugestoes_depara_ia.json
```

As sugestões da IA não alteram documentos nem o arquivo principal automaticamente. Revise-as e copie somente as correspondências corretas para `depara_chaves.json`. O resolvedor analisa nomes de chaves, não deve ser usado para confirmar valores técnicos de catálogo e não substitui a validação do técnico.
...
Para o Memorial Descritivo, use o arquivo `MEMORIAL_DESCRITIVO_marcadores.docx`. Os campos da entrada do padrão, como bitola de cabos, disjuntor, curva e características do aterramento, devem ser fornecidos pelo técnico de campo. O técnico responsável informa o modelo, a quantidade, as potências e o tipo de arranjo dos módulos e inversores. Os demais dados de catálogo podem ser completados no TXT após OCR ou conferência em ficha técnica oficial. O programa não transforma os valores do exemplo em padrões e lista no relatório tudo o que ainda não foi confirmado.
...
Antes de protocolar, abra os arquivos gerados no Word e no Excel, confira os campos listados no relatório e valide especialmente tensão, disjuntor, coordenadas UTM, potência, modelo dos equipamentos, bitolas, aterramento e data. O preenchimento automático não substitui a conferência técnica nem a assinatura exigida.
