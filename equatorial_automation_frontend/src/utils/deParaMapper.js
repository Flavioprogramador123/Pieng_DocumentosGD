...
export function buildLocalDeParaPreview(clientData, technicalData, modules, inverters) {
  const mappings = []
  const seen = new Set()
  const push = (...args) => {
    const r = row(...args)
  ...
  push('Nº Poste/Transformador', 'NUM_POSTE', technicalData.num_poste)
...
export function mergeDeParaPreview(local, remote) {
  ...
  return {
    ...remote,
    source: 'server',
    // Garantir mappings mesmo se servidor retornar vazio
